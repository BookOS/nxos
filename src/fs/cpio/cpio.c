...
NX_PRIVATE int CpioMount(NX_VfsMount * m, const char * dev)
{
...
NX_PRIVATE int CpioUnmount(NX_VfsMount * m)
{
	m->data = NX_NULL;
	return 0;
}

NX_PRIVATE int CpioMountSync(NX_VfsMount * m)
{
	return 0;
}

NX_PRIVATE int CpioVget(NX_VfsMount * m, NX_VfsNode * n)
{
	return 0;
}

NX_PRIVATE int CpioVput(NX_VfsMount * m, NX_VfsNode * n)
{
	return 0;
}

NX_PRIVATE NX_U64 CpioRead(NX_VfsNode * n, NX_I64 off, void * buf, NX_U64 len)
{
	NX_U64 toff;
	NX_U64 sz = 0;
...
NX_PRIVATE NX_U64 CpioWrite(NX_VfsNode * n, NX_I64 off, void * buf, NX_U64 len)
{
	return 0;
}

NX_PRIVATE int CpioTruncate(NX_VfsNode * n, NX_I64 off)
{
	return -1;
}

NX_PRIVATE int CpioSync(NX_VfsNode * n)
{
	return 0;
}

NX_PRIVATE int CpioReaddir(NX_VfsNode * dn, NX_I64 off, NX_VfsDirent * d)
{
	struct CpioNewcHeader header;
...
NX_PRIVATE int CpioLookup(NX_VfsNode * dn, const char * name, NX_VfsNode * n)
{
	struct CpioNewcHeader header;
	char path[NX_VFS_MAX_PATH];
	NX_U64 off = 0, rd;
...
NX_PRIVATE int CpioCreate(NX_VfsNode * dn, const char * filename, NX_U32 mode)
{
	return -1;
}

NX_PRIVATE int CpioRemove(NX_VfsNode * dn, NX_VfsNode * n, const char *name)
{
	return -1;
}

NX_PRIVATE int CpioRename(NX_VfsNode * sn, const char * sname, NX_VfsNode * n, NX_VfsNode * dn, const char * dname)
{
	return -1;
}

NX_PRIVATE int CpioMkdir(NX_VfsNode * dn, const char * name, NX_U32 mode)
{
	return -1;
}

NX_PRIVATE int CpioRmdir(NX_VfsNode * dn, NX_VfsNode * n, const char *name)
{
	return -1;
}

NX_PRIVATE int CpioChmod(NX_VfsNode * n, NX_U32 mode)
{
	return -1;
}
