...
NX_Error HandlePageAttrFault(NX_Thread * thread, NX_Addr addr, int noWrite)
{
    if (noWrite)
    {
        NX_LOG_E("thread name=%s tid=%d page %p no write attr", thread->name, thread->tid, addr);
    }
    else
    {
        NX_LOG_E("thread name=%s tid=%d page %p no read attr", thread->name, thread->tid, addr);
    }
...
NX_Error NX_HalHandlePageFault(NX_Thread * thread, NX_HalTrapFrame * frame)
{
    ...
    {
        if (addr < vmspace->stackBottom && (addr + 4096) >= frame->esp) /* fault addr under stack, near 4096 bytes */
        {
            /* expand stack */
    ...
    {
        return HandlePageAttrFault(thread, addr, frame->errorCode & PAGE_ERR_WRITE);
    }
